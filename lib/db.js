...
export async function criarNoticia(slug, dados) {
  const { rows } = await sql`
    INSERT INTO noticias (slug, titulo, resumo, conteudo, imagem_capa, categoria, municipio, data_publicacao, publicada, destaque)
    VALUES (
    ...
    RETURNING slug, titulo, resumo, conteudo, imagem_capa AS "imagemCapa",
              categoria, municipio, to_char(data_publicacao, 'YYYY-MM-DD') AS "dataPublicacao", publicada, destaque;
  `;
  return rows[0];
}
